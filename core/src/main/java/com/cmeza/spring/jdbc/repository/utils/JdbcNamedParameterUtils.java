...
public final class JdbcNamedParameterUtils {
    ...
    private void evaluateParameterSourceValueArray(Object entryItem, StringBuilder actualSql) {
        if (entryItem instanceof Object[]) {
            actualSql.append('(');
            for (int m = 0; m < ((Object[])entryItem).length; m++) {
                if (m > 0) {
                    actualSql.append(", ");
                }
                actualSql.append('?');
    ...
    public List<ParameterProperties> extractOutParameters(List<ParameterProperties> outParameters, NamingStrategy namingStrategy) {
        if (Objects.isNull(outParameters)) return null;
        outParameters.forEach(param -> param.setValue(namingStrategy.parse(param.getValue())));
