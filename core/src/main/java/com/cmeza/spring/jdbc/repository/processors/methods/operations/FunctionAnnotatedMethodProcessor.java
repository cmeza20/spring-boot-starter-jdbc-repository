...
import com.cmeza.spring.jdbc.repository.support.parsers.methods.JdbcFunctionParser;
...
public class FunctionAnnotatedMethodProcessor extends AbstractAnnotatedMethodProcessor<JdbcFunction, JdbcFunctionProperties> {

    public FunctionAnnotatedMethodProcessor(JdbcRepositoryProperties jdbcRepositoryProperties) {
    ...
    @Override
    protected JdbcFunctionProperties dslLocator(JdbcFunction annotation, DslProperties dslProperties, ClassMetadata classMetadata, MethodMetadata methodMetadata) {
        JdbcFunctionProperties jdbcFunctionProperties = dslProperties.getFunction();
        if (Objects.isNull(jdbcFunctionProperties)) {
            jdbcFunctionProperties = new JdbcFunctionProperties();
        }

        return jdbcFunctionProperties;
    }

    @Override
    protected void dslParser(JdbcFunction annotation, JdbcFunctionProperties dsl) {
        PARSER.getParser(JdbcFunctionParser.class).parse(annotation, dsl);
    }

    @Override
    protected void resolvePlaceholders(JdbcFunctionProperties dslProperty, JdbcRepository jdbcRepository, MethodMetadata methodMetadata) {
        dslProperty.setName(propertiesResolver.resolveRequiredPlaceholders(dslProperty.getName(), JdbcMessageUtils.validateAnnotationMessage(methodMetadata, JdbcFunction.class, JdbcConstants.NAME)));
        if (dslProperty.getSchema().isEmpty()) {
            dslProperty.setSchema(jdbcRepository.schema());
        } else {
            dslProperty.setSchema(propertiesResolver.resolveRequiredPlaceholders(dslProperty.getSchema(), JdbcMessageUtils.validateAnnotationMessage(methodMetadata, JdbcFunction.class, JdbcConstants.SCHEMA)));
        }
        if (dslProperty.getCatalog().isEmpty()) {
            dslProperty.setCatalog(jdbcRepository.catalog());
        } else {
            dslProperty.setCatalog(propertiesResolver.resolveRequiredPlaceholders(dslProperty.getCatalog(), JdbcMessageUtils.validateAnnotationMessage(methodMetadata, JdbcFunction.class, JdbcConstants.CATALOG)));
        }
    ...
    @Override
    protected void updateValues(Map<String, Object> values, JdbcFunctionProperties dslProperty) {
