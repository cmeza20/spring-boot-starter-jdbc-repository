...
public class DefaultUpdateFactory implements JdbcUpdateFactory {
    ...
    public JdbcUpdateFactory withFromTable(TableDefinition tableDefinition) {
        if (Objects.nonNull(tableDefinition)) {
            ...
            this.update.setFromItem(PARSER.getParser(JdbcFromTableParser.class).parseToTable(tableDefinition));
        }
        return this;
    }
    ...
    public JdbcUpdateFactory withJoinTable(JoinTableDefinition tableDefinition) {
        if (Objects.nonNull(tableDefinition)) {
            ...
            JdbcJoinTableParser jdbcJoinTableParser = PARSER.getParser(JdbcJoinTableParser.class);

            if (tableDefinition.getPosition().equals(JdbcJoinTable.JoinPosition.NORMAL)) {
                this.update.addJoins(jdbcJoinTableParser.parseToJoin(tableDefinition));
            } else {
                this.update.addStartJoins(jdbcJoinTableParser.parseToJoin(tableDefinition));
            }
        }
        return this;
    }
        ...
        if (Objects.nonNull(tableDefinitions)) {
            Arrays.stream(tableDefinitions).forEach(t -> {
                ...
                if (t.getPosition().equals(JdbcJoinTable.JoinPosition.NORMAL)) {
                    this.update.addJoins(jdbcJoinTableParser.parseToJoin(t));
                } else {
                    this.update.addStartJoins(jdbcJoinTableParser.parseToJoin(t));
                }
            });
        }
        return this;
    ...
    public JdbcUpdateFactory withJoinTable(String table) {
        ...
        this.queryHashcode.append(table);
        this.update.addJoins(PARSER.getParser(JdbcJoinTableParser.class).parseToJoin(tableDefinition));
        return this;
    }
    ...
    public UpdateDefinition generateDefinition() {
        UpdateDefinition updateDefinition = PARSER.getParser(JdbcUpdateParser.class).parseFromUpdate(this.update);
